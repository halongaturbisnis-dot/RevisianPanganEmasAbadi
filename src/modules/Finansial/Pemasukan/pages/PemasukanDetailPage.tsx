...
/**
 * PEMASUKAN DETAIL PAGE
 * Menampilkan detail transaksi pemasukan secara mendalam.
 */
export const PemasukanDetailPage: React.FC = () => {
  ...
  const loadDetail = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await pemasukanService.getById(id);
      if (result) {
        setData(result);
  ...
  const proofFiles: IPemasukanFile[] = JSON.parse(data.proof_urls || '[]');
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 flex flex-col h-full">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                <h3 className="!text-FontSizeNano font-black text-TextColorBase opacity-80 uppercase !leading-none tracking-widest pl-SpacingNano border-l-2 border-ColorPrimary flex items-center gap-SpacingNano">Bukti Pemasukan</h3>
              </div>
...
export default PemasukanDetailPage;
